...
namespace tcs_hw {
	...
	namespace fsa {
		...
		namespace {
			const std::regex state_regex("[a-zA-Z0-9]+");
			const std::regex alpha_regex("\\w+");
		}
	}

	namespace err {
		const char* const E1 = "E1: Input file is malformed";
		const char* const E2 = "E2: Initial state is not defined";
		const char* const E3 = "E3: Set of accepting states is empty";
		const char* const E6 = "E6: Some states are disjoint";
		const char* const E7 = "E7: FSA is nondeterministic";
	}
	...
	template <typename P> [[nodiscard]] inline result<std::set<std::string>> parse_args(
		...
		while (std::getline(input_stream, input, ',')) {
			if (!predicate(input)) return { err::E1 };
			args.insert(input);
		}
	...
	inline void panic_state_not_found(std::ostream& out, const fsa::state& state) noexcept {
		PANIC(out, "E4: A state \'" << state << "\' is not in the set of states");
	}
	...
	inline void panic_alpha_not_found(std::ostream& out, const fsa::alpha& alpha) noexcept {
		PANIC(out, "E5: A transition \'" << alpha << "\' is not represented in the alphabet");
	}
	...
	inline bool is_state_name_correct(const fsa::state& state) noexcept {
		return std::regex_match(state, fsa::state_regex);;
	}
	...
	inline bool is_alpha_name_correct(const fsa::alpha& alpha) noexcept {
		return std::regex_match(alpha, fsa::alpha_regex);;
	}
	...
	[[nodiscard]] inline fsa::state parse_init_state_or_panic(
		...
		auto parse_init_result = parse_args(in, "initial=[");
		...
		if (init_states.empty()) { PANIC(out, err::E2); }
		if (init_states.size() > 1) { PANIC(out, err::E1); }
	...
	[[nodiscard]] inline std::set<fsa::state> parse_final_states_or_panic(
		...
		auto final_states = tcs_hw::parse_args_or_panic(in, out, "accepting=[");
		...
		if (final_states.empty()) { PANIC(out, err::E3); }
		return final_states;
	}
	...
	[[nodiscard]] inline fsa::transition parse_transition_or_panic(
		...
		if (!std::getline(input_stream, start, '>')) { PANIC(out, err::E1); }
		if (!std::getline(input_stream, alpha, '>')) { PANIC(out, err::E1); }
		if (!std::getline(input_stream, finish, '>')) { PANIC(out, err::E1); }
		if (!input_stream.eof()) { PANIC(out, err::E1); }
	...
	inline void validate_fsa(
		...
		// return is_fsa_complete(state_machine);
	}

	inline auto to_ssa_trans(const fsa::transitions& s_a_s_trans) noexcept {
		std::map<fsa::state, std::map<fsa::state, std::set<fsa::alpha>>> s_s_a_trans;

		for (const auto& [s, a_s] : s_a_s_trans)
			for (const auto& [a, ss] : a_s)
				for (const auto& to_s : ss)
					s_s_a_trans[s][to_s].insert(a);

		return s_s_a_trans;
	}

	/**
	 * Converts FSA to regular expression with Kleene's algorithm
	 * @param state_machine FSA to convert
	 * @return resulting regex
	 */

	inline std::string to_regex(const fsa::fsa& state_machine) noexcept {
		const auto& states = std::get<fsa::fsa_args::STATES>(state_machine);
		const auto states_number = states.size();

		const auto& init_state = std::get<fsa::fsa_args::INITIAL_STATE>(state_machine);
		const auto& accepting_states = std::get<fsa::fsa_args::FINAL_STATES>(state_machine);
		const auto& s_a_s_trans = std::get<fsa::fsa_args::TRANSITIONS>(state_machine);
		auto&& s_s_a_trans = to_ssa_trans(s_a_s_trans);

		std::vector<std::map<fsa::state, std::map<fsa::state , std::string>>> kleene(states_number + 1);

		for (const auto& i_state : states) {
			for (const auto& q_state : states) {
				const auto st1_it = s_s_a_trans.find(i_state);

				if (st1_it == s_s_a_trans.end()) {
					kleene[0][i_state][q_state] = i_state != q_state ? "{}" : "eps";
					continue;
				}

				const auto& st1_trans = st1_it->second;
				const auto st2_it = st1_trans.find(q_state);

				if (st2_it == st1_trans.end()) {
					kleene[0][i_state][q_state] = i_state != q_state ? "{}" : "eps";
					continue;
				}

				const auto& st2_trans = st2_it->second;
				std::stringstream acc;
				acc << *st2_trans.begin();

				std::for_each(
					std::next(st2_trans.begin()),
					st2_trans.end(),
					[&acc](const auto& tr) { acc << "|" << tr; }
				);

				if (i_state == q_state)
					acc << "|eps";

				kleene[0][i_state][q_state] = acc.str();
			}
		}
		...
		std::for_each(
			states.begin(),
			states.end(),
			[k = 0, &kleene, &states](const auto& k_state) mutable {
				for (const auto& i_state : states) {
					for (const auto& q_state : states) {
						std::stringstream acc;
						acc << "(" << kleene[k][i_state][k_state] << ")";
						acc << "(" << kleene[k][k_state][k_state] << ")*";
						acc << "(" << kleene[k][k_state][q_state] << ")|";
						acc << "(" << kleene[k][i_state][q_state] << ")";
						kleene[k + 1][i_state][q_state] = acc.str();
					}
				}
		...
		std::stringstream accepting_acc;
		accepting_acc << "(" << kleene.back()[init_state][*accepting_states.begin()] << ")";

		std::for_each(
			std::next(accepting_states.begin()),
			accepting_states.end(),
			[&accepting_acc, &kleene, &init_state](const auto& as) {
				accepting_acc << "|(" << kleene.back()[init_state][as] << ")";
			}
		);

		return accepting_acc.str();
	}
}
...
int main() {
	...
	const auto fsa = tcs_hw::parse_fsa_or_panic(in, std::cout);
	in.get();

	if (!in.eof()) {
		PANIC(std::cout, tcs_hw::err::E1);
	}

	tcs_hw::validate_fsa(fsa, std::cout);
	std::cout << tcs_hw::to_regex(fsa);
